...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, conv_kwargs=None,
        ...
        # upsample will scale the number of channels by up_size, 
        # so we need to halve the number of channels in the input
        conv1_in_channels = in_channels // up_size + self.x_skip_channels #// up_size removed from after in_channels
        ...
        if up_type == 'interpolate':
            self.upsample = nn.Sequential(nn.Upsample(scale_factor=up_size, mode='bilinear', align_corners=False),
                                          nn.Conv2d(in_channels, in_channels // up_size, kernel_size=1, stride=1)) #// up_size removed from second arg
        elif up_type == 'convtranspose':
            self.upsample = nn.ConvTranspose2d(in_channels, in_channels // up_size, kernel_size=2, stride=up_size)
        elif up_type == 'conv_then_interpolate':
            self.upsample = nn.Sequential(nn.Conv2d(in_channels, in_channels // up_size, kernel_size=1, stride=1), #// up_size removed from second arg
        ...
        self.use_resid_connection = use_resid_connection
        if use_resid_connection:
            self.resid_connection = nn.Conv2d(conv1_in_channels, out_channels, kernel_size=1, stride=1) #// up_size removed from first arg
    ...
    def forward(self, x, x_skip):
        ...
        if self.use_resid_connection:
            x_resid = self.resid_connection(x)
        #print("Skip", x.shape)
        x_out = self.conv1(x)
        x_out = self.conv2(x_out)
...
class SelfAttention(nn.Module):
    def __init__(self, embed_dim=128, dim_size=12, up_type="interpolate", att_kwds={}):
        super().__init__()
        self.embed_dim = embed_dim
        self.dim_size = dim_size
        self.up_type = up_type
        
        self.pos_embed = nn.Parameter(torch.randn((1, self.dim_size**2, self.embed_dim))*.02)
        self.attention = vit.Block(dim=self.embed_dim, **att_kwds)
        self.align_deep_res = self.get_resizer(dim_size)

    def get_resizer(self, dim_size):
        if self.up_type == "interpolate":
            align_resolution = nn.UpsamplingBilinear2d(size=dim_size)
        elif self.up_type == 'interpolate_then_conv':
            align_resolution = nn.Sequential(nn.UpsamplingBilinear2d(size=dim_size),
                                             nn.Conv2d(self.embed_dim, self.embed_dim, kernel_size=1, stride=1))
        elif self.up_type == 'conv_then_interpolate':
            align_resolution = nn.Sequential(nn.Conv2d(self.embed_dim, self.embed_dim, kernel_size=1, stride=1),
                                             nn.UpsamplingBilinear2d(size=dim_size))
        return align_resolution

    def forward(self, x):
        B, C, H, W = x.shape

        # handling multi-resolution, and setting up for upsampling after block
        if (H, W) != self.dim_size: 
            x = self.align_deep_res(x)
            correct_resolution = self.get_resizer((H, W))

        # Reshape into (B, P, C)
        x = x.reshape(B, C, self.dim_size**2).swapaxes(1,2)

        # Add positional embeddings and apply attention
        x = x + self.pos_embed
        x = self.attention(x)

        # Reshape back and upsample back to correct dims for multi-resolution
        x = x.swapaxes(1, 2).reshape(B, C, self.dim_size, self.dim_size)
        if x.shape[-2:] != (H, W):
           x = correct_resolution(x)

        def __repr__(self):
            return f'SelfAttention(embedding_dims={self.embed_dim}, heads={self.num_heads})'
...
class UNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, depth=4, channels: [int, str, list] = 32,
                 ...
                 pool_layer=nn.MaxPool2d, block_factor=2, up_type='interpolate', self_attention=False, attention_cf=None,
        ...
        #     if attention_cf is not None:
        #         self.attention = SelfAttention(patch_size, embed_dim, feat_size=(feat_size, feat_size),**attention_cf)
        #     else:
        ...
        if self.self_attention:
            embed_dim = channels[-1] # number of filters at deepest point of network
            dim_size = int((768 // 2**np.cumsum(np.log2(self.block_factor)))[-1])
            if attention_cf is None:
                attention_cf = {"num_heads":8, "mlp_ratio":4.0,"proj_drop":0.0, "attn_drop":0.0, "drop_path":0.0}
            self.attention = SelfAttention(embed_dim, dim_size, "interpolate", att_kwds=attention_cf)
