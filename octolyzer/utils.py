...
def load_dcmfile(dcm_oct_path, dcm_slo_path, preprocess=False, custom_maps=[], logging=[], verbose=True):
    ...
    slo_acq = slo_acq[int(max(0,slo_minmax_y[0])):int(min(slo_N,slo_minmax_y[1])), int(max(0,slo_minmax_x[0])):int(min(slo_N,slo_minmax_x[1])+0.5), :]
    ...
    # superimpose the acquisition line onto a copy of the en face SLO.
        ...
        for idx, point in enumerate(all_px_points):
            ...
            for (x,y) in zip(x_grid, y_grid):
                ...
                # Overlay acquisition locations
                if (0 <= x_idx < slo_acq.shape[1]) & (0 <= y_idx < slo_acq.shape[0]):
                    slo_acq[y_idx, x_idx] = loc_colour
                slo_acq_fixed[pad_y[0]+y_idx, pad_x[0]+x_idx] = loc_colour
