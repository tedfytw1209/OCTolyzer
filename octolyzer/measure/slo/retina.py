...
from octolyzer.measure.slo.function_ import thinning
...
class Window(Retina):
    ...
    #     '''
    #     30/05/2024: Given we always measure the entire input x, create_windows has been significantly simplified.

    #     This is because while create_windows_NEW was a simpler implementation than create_windows_OLD, it would ocassionally
    #     fail unexpectedly. Thus, this simpler version has been used instead.
    #     '''
    #     # Output entire input image, with positions as the top-left and bottom-right locations.
    #     roi = np.array(x.np_image)
    #     H, W = roi.shape
    #     x_pos = np.array([[0,0], [W, H]])
    #     x = roi.reshape(1,1,W,H)
    ...
    def create_windows(x: Retina, dimension, method="separated", min_pixels=10) -> tuple:
        ...
        H, W = x.shape
        shape = (H - d1 + 1, W - d2 + 1, # N_strides vertically, horizontally
                 d1, d2) # Patch size vertically, horizontally
        H_str, W_str = x.stride(dim=-2), x.stride(dim=-1)
        strides = (H_str, W_str, H_str, W_str)
    
        # Patch the input
        x = torch.as_strided(x, size=shape, stride=strides)
        x = x[::d1, ::d2] # x is (batch_size, filters, patchH, patchW, patch_size, patch_size)
    
        # remove any with less than min_pixels
        x = x.reshape(-1, 1, d1, d2).numpy()
    
        # get positions
        N_pheight = int(H // d1)
        N_pwidth = int(W // d2)
        x_pos = np.array([np.array([[i*d1,j*d2],
                                    [(i+1)*d1,(j+1)*d2]]) 
                          for i in range(N_pheight) 
